...
<html lang="en">
	<head>
		...
		<!-- STEP 2: Connect to your Web server, and upload all of the files in this lab to a new folder called 
		'lab4' inside the 'week4' directory we created during this week's lesson -->
		<!-- STEP 3: Make sure that you can browse to this page on the server - 
		the URL that you would have to enter into the address bar in a browser
		 might look something like https://mysitename.azurewebsites.net/week4/lab4/index.php (for Microsoft Azure) -->
		 <!-- Created the Lab04SGouveia under XAMPP/HTDOCS/WEEK4-->
	</head>
	<body>
		<!-- Page-level header -->
		<header>
...
<!-- STEP 5: Add an action attribute to the FORM start tag that points to a file called "subscribe.php", 
and set the method attribute to "get" -->
			<form action="subscribe.php" method=GET>     	<!--Opening the FORM -->
<!-- STEP 6: Add an H3 element inside the FORM tags that reads, "Subscribe to Newsletter" -->
				<h3>Subscribe to Newsletter</h3> 			<!-- Created the heading3 -->
<!-- STEP 7a: Add a LABEL element for the email address -->
				<label for="emailAddress">Email:</label>	<!--added Label, the for attribute -->
<!-- STEP 7b: Add an INPUT element for the user to type their email address - set the type of INPUT to "email", 
add a name attribute that is set to "emailAddress", and an id attribute that is set to "emailAddress" -->
				<input type="email" id="emailAddress" name="emailAddress" required/> 	<!--Added the input, required (7d)-->
<!-- STEP 7c: Add the 'for' attribute to the above LABEL element, and set the value to match the 'id' 
value of the INPUT element -->
<!-- STEP 7d: Include the 'required' attribute on the above INPUT element so that users are forced to enter an 
email address before the form is submitted -->
<!-- STEP 8a: Add a BUTTON element with the attribute type="submit" -->
				<button type="submit"> Subscribe </button>		<!--Added the button with Subscribe -->
<!-- STEP 8b: Set the content of the BUTTON element to read, "Subscribe" -->
<!-- STEP 8c: Upload the changed index.php file that you've now edited to the server and refresh the page 
to view the changes - try adding an email address and submitting the form -->
				<!-- Sent the email and the page is redirecting to http://localhost/week4/Lab4/subscribe.php?emailAddress=sergiocutrim%40gmail.com-->
				<!-- The opened page has Thank you for Subscribing -->
